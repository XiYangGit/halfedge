...
ShaderProgram::ShaderProgram(GLWidget277 *context)
    : vertShader(), fragShader(), prog(),
      ...
      unifTime(-1),unifRenderMode(-1),
      unifBindMatrices(-1),unifJointTransformations(-1),
      attrInfluencerID(-1),attrWeights(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    //added by Xi Yang
    attrInfluencerID=context->glGetAttribLocation(prog, "vs_Vertex_Influencer_IDs");
    attrWeights=context->glGetAttribLocation(prog,"vs_Vertex_Weights");
    unifJointTransformations=context->glGetUniformLocation(prog, "u_Joint_Transformations");
    unifBindMatrices=context->glGetUniformLocation(prog, "u_Bind_Matrices");
...
void ShaderProgram::setUnifTime(int t)
{
    useMe();
...
void ShaderProgram::SetUnifRenderMode(int i)
{
    useMe();

    if(unifRenderMode != -1)
    {
        context->glUniform1i(unifRenderMode,i);
    }
}
//added by Xi Yang
void ShaderProgram::SetUnifBindMatrices(const glm::mat4 (&i)[100])
{
    useMe();

    if(unifBindMatrices != -1)
    {
        context->glUniformMatrix4fv(unifBindMatrices,100,GL_FALSE,&i[0][0][0]);
    }
}
void ShaderProgram::SetUnifJointTransformations(const glm::mat4 (&i)[100])
{
    useMe();

    if(unifJointTransformations != -1)
    {
        context->glUniformMatrix4fv(unifJointTransformations,100,GL_FALSE,&i[0][0][0]);
    }
}
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    //added by Xi Yang
    if (attrInfluencerID != -1 && d.bindInf()) {
        context->glEnableVertexAttribArray(attrInfluencerID);
        context->glVertexAttribIPointer(attrInfluencerID,2,GL_INT,0,NULL);
    }
    ...
    //added by Xi Yang
    if (attrInfluencerID != -1) context->glDisableVertexAttribArray(attrInfluencerID);
